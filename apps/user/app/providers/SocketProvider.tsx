...
export function SocketProvider({ children }: SocketProviderProps) {
  ...
  // Debug logging
  useEffect(() => {
    console.log('🔍 SocketProvider Debug:', {
      status,
      hasSession: !!session,
  ...
  useEffect(() => {
    async function connectSocket() {
      console.log('🔌 connectSocket called, status:', status);
      
      // Only connect if user is authenticated and has a token
      if (status === 'authenticated' && session?.user?.token) {
        try {
          setIsConnecting(true);
          console.log('🔌 Initializing socket connection with token:', session.user.token);
        ...
        console.log('⚠️ Socket connection skipped:', {
